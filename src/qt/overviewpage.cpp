...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBugoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBugoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //2018-02-03 BUG Fix
        //strAnonymizeBugoAmount = strAnonymizeBugoAmount.remove(strAnonymizeBugoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAnonymizeBugoAmount = strAnonymizeBugoAmount.remove(strAnonymizeBugoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit));
        strAmountAndRounds = strAnonymizeBugoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBugoAmount*COIN) nMaxToAnonymize = nAnonymizeBugoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBugoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBugoAmount));
        //2018-02-03 BUG Fix
        //strAnonymizeBugoAmount = strAnonymizeBugoAmount.remove(strAnonymizeBugoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAnonymizeBugoAmount = strAnonymizeBugoAmount.remove(strAnonymizeBugoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit));
        strAmountAndRounds = strAnonymizeBugoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
