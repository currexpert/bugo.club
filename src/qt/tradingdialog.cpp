    ...
    model(0)
{
    ...
    ui->BUGOAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexBUGOLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "BUGO(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "BUGO(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(BUGO)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //BUGOst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-BUGO");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-BUGO&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-BUGO&count=100");
      return Response;
}
...
QString tradingDialog::BuyBUGO(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-BUGO&quantity=";
...
QString tradingDialog::SellBUGO(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-BUGO";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=BUGO";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-BUGO&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>BUGO Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> BUGO");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->BUGOSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(BUGOSupply,'i',8) + "</span><b> BUGO</b>");
    ...
    ui->BUGODemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(BUGODemand,'i',8) + "</span><b> BUGO</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("BUGO");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->BUGOAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("BUGO");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexBUGOLabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("BUGO");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->BUGOBalanceLabel,*ui->BUGOAvailableLabel_2,*ui->BUGOPendingLabel, QString::fromUtf8("BUGO"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputBUGO->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("BUGO");
    ...
    double AvailableBUGO = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling BUGO available balance
    QString responseA = GetBalance("BUGO");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableBUGO = ResultObject["Available"].toDouble();

    ui->UnitsInputBUGO->setText(str.number(AvailableBUGO,'i',8));
}
...
{
    double Quantity = ui->BittrexBUGOLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableBUGO,'i',8));
}
...
void tradingDialog::on_BuyBUGO_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyBUGO(Order,Quantity,Rate);
...
void tradingDialog::on_SellBUGOBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputBUGO->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellBUGO(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellBUGO(Order,y,x);
                ...
                QString SellResponse = SellBUGO(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" BUGO for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" BUGO for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputBUGO_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
