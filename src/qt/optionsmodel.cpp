...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBugoAmount"))
        settings.setValue("nAnonymizeBugoAmount", 1000);
    nAnonymizeBugoAmount = settings.value("nAnonymizeBugoAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBugoAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBugoAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBugoAmount:
            return QVariant(nAnonymizeBugoAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBugoAmount:
            nAnonymizeBugoAmount = value.toInt();
            settings.setValue("nAnonymizeBugoAmount", nAnonymizeBugoAmount);
            emit AnonymizeBugoAmountChanged(nAnonymizeBugoAmount);
            break;
        default:
            break;
        }
